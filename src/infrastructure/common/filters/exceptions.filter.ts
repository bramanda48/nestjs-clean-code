...
import { IExceptionMessage, IResponseFormat } from '../../../domain/common/response.interface';
...
export class AllExceptionFilter implements ExceptionFilter {
    constructor(
        ...
        // Log message
        errors.forEach(message => this.logMessage(request, message, status, exception));
    ...
    private logMessage(request: any, message: IExceptionMessage, status: number, exception: any) {
        if (status === 500) {
            this.logger.error(
                `End Request for ${request.path}`,
                `method=${request.method} status=${status} code=${message.code ?? null} message=${message.message ?? null}`,
